...
D3D::D3D(int screenWidth, int screenHeight, bool vsync, HWND hwnd, bool fullscreen, float screenDepth, float screenNear)
{
	...
	cullModeState = false; // Front cull
	cullOn = true;
...
void D3D::createDefaultRasterState()
{
	...
	device->CreateRasterizerState(&rasterDesc, &rasterStateCFront);
	deviceContext->RSSetState(rasterStateCFront);

	//set cull mode to back
	rasterDesc.CullMode = D3D11_CULL_BACK;
	device->CreateRasterizerState(&rasterDesc, &rasterStateCBack);

	//set cull mode to none
	rasterDesc.CullMode = D3D11_CULL_NONE;
	device->CreateRasterizerState(&rasterDesc, &rasterStateCOff);
...
void D3D::createBlendState()
{
	...
	blendStateDescription.RenderTarget[0].SrcBlend = D3D11_BLEND_ONE; // D3D11_BLEND_ONE
...
D3D::~D3D()
{
	...
	if (rasterStateCFront)
	{
		rasterStateCFront->Release();
		rasterStateCFront = 0;
	}
...
// Alter the cullmode from back to front
void D3D::setCullBack(bool b)
{
	cullModeState = b;
	if (cullModeState) // If true, set back cull
	{
		deviceContext->RSSetState(rasterStateCBack);
	}
	else // If false, set front cull
	{
		deviceContext->RSSetState(rasterStateCFront);
	}
}
...
void D3D::setCullOn(bool b)
{
	cullOn = b;
	if (!cullOn) // Turn off culling completely
	{
		deviceContext->RSSetState(rasterStateCOff);
	}
	else // default
	{
		deviceContext->RSSetState(rasterStateCFront);
